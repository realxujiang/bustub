...
class SimpleCatalog {
 public:
  ...
  TableMetadata *GetTable(const std::string &table_name) {
    auto check = names_.find(table_name);
    if (check == names_.end()) {
      throw std::out_of_range("Table name doesn't exist.");
    }

    return GetTable(check->second);
  }

  /** @return table metadata by oid */
  TableMetadata *GetTable(table_oid_t table_oid) {
    auto check = tables_.find(table_oid);
    if (check == tables_.end()) {
      throw std::out_of_range("Table oid doesn't exist.");
    }
